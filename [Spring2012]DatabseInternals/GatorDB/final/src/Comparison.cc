...
Type* OrderMaker :: GetAttributeTypes()
{
    return whichTypes;
}
...
void CNF::GetQueryOrderMaker(OrderMaker &qom, OrderMaker &lom, OrderMaker &fom)
{
    qom.numAtts=0;
    lom.numAtts=0;
    for(int i=0;i<fom.numAtts;i++)
    {
        for(int j=0;j<numAnds;j++)
        {
            ...
            if(orList[j][0].whichAtt1 == fom.whichAtts[i])
            {
                qom.numAtts++;
                qom.whichAtts[i]=fom.whichAtts[i];
                qom.whichTypes[i]=fom.whichTypes[i];
                lom.numAtts++;
                lom.whichAtts[i]=orList[j][0].whichAtt2;
                lom.whichTypes[i]=fom.whichTypes[i];
            }
        }
        if(qom.numAtts!=i+1)
        {
